...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    HeapPQueue priorityQueue;
    Map<string, int> seats = populations;
    ...
    for (auto& entry : populations) {
        int firstIteration = -double(populations[entry]/sqrt(2));
        priorityQueue.enqueue((DataPoint(entry, firstIteration)));
    }

    // while loop until seats = 0, updating the map seats
    // how to dequeue the highest number?
    while (numSeats > 0) {

        DataPoint state = priorityQueue.dequeue();
        seats[state.name]++;
        int S = seats[state.name];
        int weight = -populations[state.name]/sqrt(S*(S+1));

        priorityQueue.enqueue((DataPoint(state.name, weight)));
        numSeats--;
    }
...
PROVIDED_TEST("Works on the 2020 apportionment.") {
    auto populations = populationsFrom("res/apportionment/2020-NYUp89.csv");

    EXPECT_NO_ERROR(apportion(populations, 435));
