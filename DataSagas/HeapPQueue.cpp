...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    if (logicalSize+1 == allocatedSize) {
        DataPoint* newElems = new DataPoint[allocatedSize*2];
    ...
    if (logicalSize == 0) {
        ...
        DataPoint* child_temp = &elems[n];
        DataPoint* parent_temp = &elems[n/2];

        while (n/2 >= 1 && child_temp->weight < parent_temp->weight) {
            swap(elems[n], elems[n/2]);
            n = n/2;
            child_temp = &elems[n];
            parent_temp = &elems[n/2];
        }
...
DataPoint HeapPQueue::dequeue() {
    ...
    DataPoint toBeReturned = elems[1];
    // swap 2 nodes
    swap(elems[1], elems[n]);
    ...
    DataPoint* parent_temp = &elems[p];
    DataPoint* child_left = &elems[left(p)];
    DataPoint* child_right = &elems[right(p)];
    // return the node
    while ((right(p) <= l || left(p) <= l) && (child_left->weight <= parent_temp->weight || child_right->weight <= parent_temp->weight)) {
        if (right(p) <= l && child_right->weight <= child_left->weight) {
           swap(elems[right(p)], elems[p]);
           p = right(p);
           parent_temp = &elems[p];
           child_left = &elems[left(p)];
           child_right = &elems[right(p)];
        }
        else if (child_left->weight < parent_temp->weight || child_left->weight <= child_right->weight)
        {
            swap(elems[left(p)], elems[p]);
            p = left(p);
            parent_temp = &elems[p];
            child_left = &elems[left(p)];
            child_right = &elems[right(p)];
